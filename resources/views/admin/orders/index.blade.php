    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                ...
                <p class="text-slate-600 font-medium mt-1">Consultez l'historique complet des transactions.</p>
            </div>
            <div class="flex items-center gap-3 bg-white p-2 rounded-2xl shadow-sm border border-slate-100">
                <div class="px-4 py-2 bg-amber-50 rounded-xl text-center">
                    <div class="text-[10px] font-black text-amber-400 uppercase tracking-widest">Total Ventes</div>
                    <div class="text-lg font-black text-amber-700">{{ $orders->where('status', 'success')->count() }}</div>
                </div>
                        ...
                        <tbody class="divide-y divide-slate-50">
                            @forelse($orders as $order)
                                <tr class="group hover:bg-slate-50/50 transition-colors duration-300">
                                    <td class="px-8 py-6">
                                        <span class="text-xs font-black text-slate-500 font-mono bg-slate-100 px-2 py-1 rounded-md">
                                    ...
                                    <td class="px-8 py-6">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div class="text-xs text-slate-500 font-medium">{{ $order->client_email ?? $order->user->email }}</div>
                                    ...
                                    <td class="px-8 py-6">
                                        <div class="font-black text-slate-900 text-sm">
                                            {{ number_format($order->amount, 0, '', ' ') }} <span class="text-[10px] text-slate-500">CFA</span>
                                        </div>
                                    </td>
                                    <td class="px-8 py-6">
                                ...
                                <tr>
                                    <td colspan="6" class="px-8 py-20 text-center">
                                        <div class="flex flex-col items-center gap-4">
                                            ...
                                            <div class="text-slate-500 font-bold text-sm tracking-wide uppercase">Aucune vente enregistrée.</div>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
