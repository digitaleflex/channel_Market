    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                ...
                <p class="text-slate-600 font-medium mt-1">Créez un nouveau produit digital pour votre marketplace.</p>
            </div>
            ...
            <div class="card-premium">
                <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="p-8 sm:p-12" x-data="{ productType: 'file' }">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                        <div class="space-y-8">
                            <div>
                                <label class="block text-xs font-black uppercase tracking-widest text-slate-500 mb-3">Titre du produit</label>
                            ...
                            <div>
                                <label class="block text-xs font-black uppercase tracking-widest text-slate-500 mb-3">Chariow Product ID</label>
                                <input type="text" name="chariow_product_id" class="input-premium" value="{{ old('chariow_product_id') }}" placeholder="Ex: prd_abc123xyz">
                                <p class="text-xs text-slate-500 mt-2">ID du produit Chariow utilisé pour le checkout API.</p>
                                @error('chariow_product_id') <span class="text-rose-500 text-xs font-bold mt-2 block">{{ $message }}</span> @enderror
                            </div>

                            <div>
                                <label class="block text-xs font-black uppercase tracking-widest text-slate-500 mb-3">Description complète</label>
                            ...
                            <div>
                                <label class="block text-xs font-black uppercase tracking-widest text-slate-500 mb-3">Prix de vente (CFA)</label>
                                <div class="relative">
                                    <input type="number" step="1" name="price" required class="input-premium pl-12" value="{{ old('price') }}" placeholder="0">
                                    <div class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 font-bold">CFA</div>
                                </div>
                        ...
                        <div class="space-y-8">
                            <div class="p-6 rounded-3xl bg-amber-50/50 border border-amber-100/50">
                                <label class="block text-xs font-black uppercase tracking-widest text-amber-400 mb-4">Produit Numérique à vendre</label>
                                
                                <div class="flex gap-4 mb-6">
                                    <label class="flex-1 cursor-pointer group">
                                        <input type="radio" name="product_type" value="file" x-model="productType" class="hidden">
                                        <div class="p-3 rounded-xl border-2 text-center transition-all" :class="productType === 'file' ? 'border-amber-500 bg-white text-amber-600 shadow-md' : 'border-slate-100 text-slate-500 group-hover:border-slate-200'">
                                    ...
                                    <label class="flex-1 cursor-pointer group">
                                        ...
                                        <div class="p-3 rounded-xl border-2 text-center transition-all" :class="productType === 'link' ? 'border-amber-500 bg-white text-amber-600 shadow-md' : 'border-slate-100 text-slate-500 group-hover:border-slate-200'">
                                ...
                                <div x-show="productType === 'file'" x-transition>
                                    <div class="relative group">
                                        ...
                                        <div class="border-2 border-dashed border-amber-200 rounded-2xl p-8 text-center group-hover:bg-white group-hover:border-amber-400 transition-all duration-300">
                                ...
                                <div x-show="productType === 'link'" x-transition x-cloak>
                                    ...
                                    <p class="text-[10px] text-slate-500 mt-2 font-bold uppercase tracking-wider">Assurez-vous que le lien est accessible aux acheteurs.</p>
                                    @error('drive_link') <span class="text-rose-500 text-xs font-bold mt-2 block">{{ $message }}</span> @enderror
                                </div>
                            </div>

                            <div>
                                <label class="block text-xs font-black uppercase tracking-widest text-slate-500 mb-3">Image de présentation (Obligatoire)</label>
                                <div class="space-y-4">
                                    <input type="file" name="image_file" required class="w-full text-sm text-slate-600 file:mr-4 file:py-2.5 file:px-6 file:rounded-xl file:border-0 file:text-xs file:font-black file:bg-slate-900 file:text-white hover:file:bg-slate-800 cursor-pointer transition shadow-lg shadow-slate-900/10">
                                </div>
                    ...
                    <div class="flex items-center justify-end gap-4 mt-12 pt-8 border-t border-slate-50">
                        <a href="{{ route('admin.products.index') }}" class="font-bold text-slate-500 hover:text-slate-600 transition-colors px-6">Annuler</a>
